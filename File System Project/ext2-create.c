...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group = 8 * 1024;
	superblock.s_frags_per_group = 8 * 1024;
...
void write_block_bitmap(int fd)
{
	...
	//loop through blocks and fill them
...
void write_root_dir_block(int fd)
{
    ...
    bytes -= lost_and_found.rec_len;

	// hello-world
    struct ext2_dir_entry hello_world = {0};
    dir_entry_set(hello_world, HELLO_WORLD_INO, "hello-world");
    dir_entry_write(hello_world, fd);
    bytes -= hello_world.rec_len;

	//hello (symlink)
    struct ext2_dir_entry hello = {0};
    dir_entry_set(hello, HELLO_INO, "hello");
    dir_entry_write(hello, fd);
    bytes -= hello.rec_len;
...
void write_hello_world_file_block(int fd)
{
    ...
    char zero_buffer[BLOCK_SIZE - len];
    if (write(fd, zero_buffer, BLOCK_SIZE - len) != BLOCK_SIZE - len) {
        errno_exit("write error");
    }
}
