...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	// adding error check for mutex function + initiation of mlock
	int err_code = pthread_mutex_init(&hash_table->mlock, NULL);
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	// adding error check for mutex function + acquisition of mlock
	int err_code = pthread_mutex_lock(&hash_table->mlock);
	// if err_code not 0, an error occurred so exit
	if (err_code != 0) {
		exit(err_code);
	}
	...
	if (list_entry != NULL) {
		...
		// adding error check for mutex function + unlocking of mlock
		int err_code = pthread_mutex_unlock(&hash_table->mlock);
		// if err_code not 0, an error occurred so exit
		if (err_code != 0) {
			exit(err_code);
		}
	...
	// this runs if the value didnt exist
	list_entry = calloc(1, sizeof(struct list_entry));
	list_entry->key = key;
	list_entry->value = value;
	SLIST_INSERT_HEAD(list_head, list_entry, pointers);
	// adding error check for mutex function + unlocking of mlock
	err_code = pthread_mutex_unlock(&hash_table->mlock);
	// if err_code not 0, an error occurred so exit
	if (err_code != 0) {
		exit(err_code);
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	// adding error check for mutex function + destruction of mlock
	int err_code = pthread_mutex_destroy(&hash_table->mlock);
	// if err_code not 0, an error occurred so exit
	if (err_code != 0) {
		exit(err_code);
	}
